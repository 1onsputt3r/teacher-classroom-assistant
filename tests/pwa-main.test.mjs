...
test('離線殼預先快取 HTML 實際引用的同版 V2 資產', async () => {
  ...
  assert.match(previewApp, new RegExp(`from '\\.\\/timer\\.mjs\\?v=${token}'`));
  for (const asset of [
    `./preview-v2/styles.css?v=${token}`,
    `./preview-v2/app.js?v=${token}`,
    `./preview-v2/core.mjs?v=${token}`,
    `./preview-v2/timer.mjs?v=${token}`
...
test('正式建置不再把舊根版 app、core 與 styles 發布為主程式', async () => {
  ...
  assert.match(build, /const previewV2Files = \['index\.html', 'styles\.css', 'app\.js', 'core\.mjs', 'timer\.mjs'\]/);
